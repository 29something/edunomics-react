...
export class VerifiedTestSix extends Component {
    constructor() {
        super();
        this.state = {
          rf22data: [],
      ...
      async handleClick() {
        ...
        console.log("value", tokenvalue);
        try {
          const rf22response = await axios.get(
            `${API_URL}rf_form/approved-rf/rf_22`,
            (axios.defaults.headers.common["x-access-token"] = tokenvalue)
          );
    
          console.log(rf22response);
    
          console.log(rf22response.data);
          console.log("rf-22 data", rf22response.data);
          if (rf22response.status === 200) {
            this.setState({ rf22data: rf22response.data });
          }
        } catch (error) {
          console.log(error);
        }
                                              ...
                                              .map(function(item, id) {
                                                return (
                                                  ...
                                                  >
                                                    ...
                                                    <td>
                                                <Link to={process.env.PUBLIC_URL + `/ViewRf22/${item.id}`}><button  className="btn btn-success custom-edit-btn btn-sm" ><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;&nbsp;View</button></Link>
                                                  {/* {localStorage.getItem('user_type') === 'quality incharge' && <Link to={process.env.PUBLIC_URL + `/VerifyRf22/${item.id}`}><button className="btn btn-warning custom-edit-btn btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp; &nbsp; Verify</button></Link>} */}
...
export default VerifiedTestSix

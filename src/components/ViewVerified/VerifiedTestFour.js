...
export class VerifiedTestFour extends Component {
  constructor() {
    super();
    this.state = {
      rf19data: [],
  ...
  async handleClick() {
    ...
    console.log("value", tokenvalue);
    try {
      const rf19response = await axios.get(
        `${API_URL}rf_form/approved-rf/rf_19`,
        (axios.defaults.headers.common["x-access-token"] = tokenvalue)
      );

      console.log(rf19response);

      console.log(rf19response.data);
      console.log("rf-19 data", rf19response.data);
      if (rf19response.status === 200) {
        this.setState({ rf19data: rf19response.data });
      }
    } catch (error) {
      console.log(error);
    }
                                          ...
                                          .map(function(item, id) {
                                            return (
                                              ...
                                              >
                                                ...
                                                <td>
                                                <Link to={process.env.PUBLIC_URL + `/ViewRf19/${item.id}`}><button  className="btn btn-success custom-edit-btn btn-sm" ><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;&nbsp;View</button></Link>
                                                  {/* {localStorage.getItem('user_type') === 'quality incharge' && <Link to={process.env.PUBLIC_URL + `/VerifyRf19/${item.id}`}><button className="btn btn-warning custom-edit-btn btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp; &nbsp; Verify</button></Link>} */}
...
export default VerifiedTestFour;

...
export class VerfiedTestSeven extends Component {
    constructor() {
        super();
        this.state = {
          rf10data: [],
      ...
      async handleClick() {
        ...
        console.log("value", tokenvalue);
        try {
          const rf10response = await axios.get(
            `${API_URL}rf_form/approved-rf/rf_10`,
            (axios.defaults.headers.common["x-access-token"] = tokenvalue)
          );
    
          console.log(rf10response);
    
          console.log(rf10response.data);
          console.log("rf-10 data", rf10response.data);
          if (rf10response.status === 200) {
            this.setState({ rf10data: rf10response.data });
          }
        } catch (error) {
          console.log(error);
        }
                                              ...
                                              .map(function(item, id) {
                                                return (
                                                  ...
                                                  >
                                                    ...
                                                    <td>
                                                <Link to={process.env.PUBLIC_URL + `/ViewRf10/${item.id}`}><button  className="btn btn-success custom-edit-btn btn-sm" ><i class="fa fa-trash-o" aria-hidden="true"></i>&nbsp;&nbsp;View</button></Link>
                                                  {/* {localStorage.getItem('user_type') === 'quality incharge' && <Link to={process.env.PUBLIC_URL + `/VerifyRF10/${item.id}`}><button className="btn btn-warning custom-edit-btn btn-sm"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp; &nbsp; Verify</button></Link>} */}
...
export default VerfiedTestSeven
